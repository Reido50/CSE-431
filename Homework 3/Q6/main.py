...
# Get number of martians how've voted
num_votes = int(input())

# Churn through all voters and store them in a bloom filter
# Calculating k came from this geeksforgeeks page: https://www.geeksforgeeks.org/bloom-filters-introduction-and-python-implementation/
p = 0.01
m = -(num_votes * log(p))/(pow(log(2), 2)) * 5
k = (m/num_votes) * log(2)
votes = BloomFilter(int(m), int(k))
for i in range(num_votes):
    voter = input().strip()
    votes.add(voter)

# Get number of new voters
num_new_votes = int(input())

# Churn through all new voters
dupes = 0
for i in range(num_new_votes):
    new_voter = input().strip()
    if votes.contains(new_voter):
        dupes += 1
